...
function testimonials_shortcode() {
    ob_start();
    // Display testimonials using the loop mentioned above
    return ob_get_clean();
}
add_shortcode( 'testimonials', 'testimonials_shortcode' );

add_filter('img_caption_shortcode', 'remove_image_caption', 10, 3);

function remove_image_caption($output, $attr, $content) {
    // Check if the $output contains the caption markup
    if (preg_match('/<figcaption/', $output)) {
        // If found, remove it
        $output = '';
    }
    return $output;
